...
public class MonitorApi extends HttpServlet {
	...
	private static final long serialVersionUID = -642084728325813546L;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			try {
				LogUtil.printInfoLog("调用查询监控数据接口", params);
				outStr = new MonitorConfigService().doMonitor(params);
			} catch (Exception e) {
				LogUtil.printErrorLog(e.getMessage(), params);
				outStr = "ERROR "+e.getMessage();
			}
			LogUtil.printInfoLog("响应: "+outStr);
