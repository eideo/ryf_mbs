...
public class MenuService {
	...
	private static List<MenuBean> accountMenus = new ArrayList<MenuBean>();
	...
	public  List<String> getAccountLoginUserFirstMenu() {
		LoginUser loginUser=sysDao.getLoginUser();
		String mid=loginUser.getMid();
		String authStr=loginUser.getAuth();//1111101001010010100101111111111
		List<MenuBean> alllist = "1".equals(mid)? getAdminFirstMenu() : getAccountFirstMenu();
		List<String> userlist = new ArrayList<String>();
		int count=0;
		for (MenuBean auth : alllist) {
			if (hasThisAuth(authStr, auth.getId())) {
	 ...
	 * @throws Exception 
	 */
	public String getAccountLeftMenu(int menuId) throws Exception {
		LoginUser loginUser=sysDao.getLoginUser();
		if(loginUser==null)throw new Exception("您未登陆，或登陆已经超时！");
//		String authStr=loginUser.getAuth();
		String authStr = genAllUserAuth("1");
//		String mid=loginUser.getMid();
		String mid="1";
		if(!"1".equals(mid))throw new Exception("对不起，您的操作权限不足！");
		return verifyAndCreatMenu(authStr, menuId, accountMenus);
	}
...
//		List<MMSNotice> noticeList=sysDao.queryMMSNotice(mid);
		...
		map.put("serviceName", "rmi_query_mmsnoticeByMid");
		map.put("mid", mid);
		Object object;
		try {
		...
		List<MMSNotice> noticeList = (List<MMSNotice>)JSONArray.toCollection(jsonArray, MMSNotice.class);
	...
	private static List<MenuBean> getAccountFirstMenu() {
		List<MenuBean> accountFirstMenu = new ArrayList<MenuBean>();
		for (MenuBean bean : accountMenus) {
			if (bean.getPid() == null) {
				accountFirstMenu.add(bean);
			}
		}
		return accountFirstMenu;
	}
	public static List<MenuBean> getAccountMenu() {
		return accountMenus;
	}
...
//		MenuBean hzcx = new MenuBean(1, 4, "M", "HZCX", " 汇总查询");
		...
		MenuBean jysjxz = new MenuBean(1, 171, "M", "JYSJXZ", "交易数据下载");
		...
		MenuBean zhlscx = new MenuBean(11, 15, "M", "ZHLSCX_28", "商户流水查询");
...
//		a_tkgl.addChild(a_tkyxsh);
		...
		MenuBean a_shjsdtb = new MenuBean(15, 148, "A", "SHJSDTB", "商户结算单同步");
...
//		MenuBean a_ZHLSCX = new MenuBean(90, 113, "A", "ZHLSCX", "账户流水查询");
		...
		MenuBean a_KJZFYHXX=new MenuBean(90,156,"A","KJZFYHXX","快捷支付用户信息查询");
...
//		adminMenus.add(a_tkgl);// 退款管理
		...
		// 账户菜单
		MenuBean zhgl = new MenuBean(1, "账户管理");
		
		MenuBean ac_WDZH = new MenuBean(1, 3, "C", "ACWOZH", "我的账户");
		MenuBean JYJLCX = new MenuBean(1, 4, "C", "ZHMMXG", "交易记录查询");
		
		MenuBean CZCX = new MenuBean(4, 1, "C", "CZCX", "充值记录查询");
		MenuBean TXCX = new MenuBean(4, 2, "C", "TXCX", "提现记录查询");
		MenuBean ZZJLCX = new MenuBean(4, 9, "C", "ZZJLCX", "转账记录查询");
		MenuBean ZHXXCX = new MenuBean(3, 8, "C", "ZHXXCX", "账户信息查询");
		MenuBean ac_ZHYE = new MenuBean(3, 6, "C", "ZHYE", "账户余额");
		MenuBean ac_ZHMMXG = new MenuBean(3, 7, "C", "ZHMMXG", "账户密码修改");
		MenuBean SZMXCX = new MenuBean(4, 5, "C", "SZMXCX", "收支明细查询");
		
		JYJLCX.addChild(CZCX);
		JYJLCX.addChild(TXCX);
		JYJLCX.addChild(ZZJLCX);
		
		ac_WDZH.addChild(ZHXXCX);
		ac_WDZH.addChild(ac_ZHYE);
		ac_WDZH.addChild(ac_ZHMMXG);
		JYJLCX.addChild(SZMXCX);
		
		zhgl.addChild(ac_WDZH);
		zhgl.addChild(JYJLCX);
		accountMenus.add(zhgl);
	}
...
//		merMenus.addAll(buttonMenus); //商户后台 不检查按钮权限和菜单权限的冲突
		...
		//增加按钮权限统计 @see	MerchantService#searchButtonAuth1(String mid,int operId)
//						merMenus.addAll(buttonMenus); //商户后台 不检查按钮权限和菜单权限的冲突
		doCount(countMap,accountMenus);
		conflictMenus = new TreeSet<MenuBean>(comparator);
		getConflictMenus(getConflictIds(countMap),accountMenus,conflictMenus);
		if(CollectionUtils.isNotEmpty(conflictMenus)){
			flag = true;
			for (MenuBean menuBean : conflictMenus) {
				logger.info("重复的账户后台菜单项  "+menuBean.getId()+"：name = "+menuBean.getName()+", " + "jsp = "+menuBean.getJsp());
			}
		}
		if(!flag){
