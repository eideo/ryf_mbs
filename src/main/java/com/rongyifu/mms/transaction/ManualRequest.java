...
public class ManualRequest{
	...
	public String requestBkUrl(Hlog hlog){
		...
		try {
			...
			LogUtil.printErrorLog("requestBkUrl", logParams, e);
		...
		try {			
			...
			LogUtil.printInfoLog("requestBkUrl", logParams);
		} catch (SocketTimeoutException e) {
			logParams.put("errorMsg", e.getMessage());
			LogUtil.printErrorLog("requestBkUrl", logParams);
			
			return "连接商户后台超时!";
		} catch (Exception e) {
			logParams.put("errorMsg", e.getMessage());
			LogUtil.printErrorLog("requestBkUrl(Hlog hlog)", logParams);
