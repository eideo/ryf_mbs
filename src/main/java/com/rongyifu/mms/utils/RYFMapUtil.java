...
public class RYFMapUtil implements java.io.Serializable {
	...
	private synchronized void refresh(String tableName) throws Exception {
		...
		if (tableName.equals(MINFO_TABLE)) {
			...
			List<Map<String, Object>> list = dao.queryForList(str_SQL);
		...
		if (tableName.equals(FEE_CALC_MODEL_TABLE)) {
			...
			List<Map<String, Object>> midGateList = dao.queryForList(sql);
	...
	private synchronized void refresh(String tableName, String mid) throws Exception {
		...
		if (tableName.equals(MINFO_TABLE)) {
			...
			String abbrev = dao.queryForString("select abbrev from minfo where id = '" + mid+"'");
	...
	private  List<Integer> queryMerGates(String mid) {
		return dao.queryForIntegerList("select gate from fee_calc_mode where  trans_mode not  in (11,12,16,17) and mid = '"+mid+"'");
	}
...
//		gateList.put(40001,new RYTGate(40001,"中国银行(银企)",4,0));
	...
	private static void initGateList() {
		List<RYTGate> aList = dao.query("select gate,stat_flag,trans_mode,gate_name from ryt_gate  ", RYTGate.class);
		gateList.clear();
		for(RYTGate o : aList){
	...
	public void refreshFeeCalcModel(String mid) {
		try {
			...
			LogUtil.printErrorLog("refreshFeeCalcModel", e);
		}
	}
	...
	public void refreshMinfoMap(String mid) {
		try {
			...
			LogUtil.printErrorLog("refreshMinfoMap", e);
		}
	}
	 ...
	 * 刷新网关信息
	 */
	public void refreshGateList(){
		try{
			initGateList();
		} catch(Exception e){
			LogUtil.printErrorLog("refreshGateList", e);
		}
	}
	 ...
	 * @return
	 */
	private static boolean filterGate(Integer transType,String gate){
		boolean res=false;
