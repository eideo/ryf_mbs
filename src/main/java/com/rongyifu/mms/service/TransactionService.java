...
import com.rongyifu.mms.bean.AccTradeInfo;
import com.rongyifu.mms.bean.AccTransferInfo;
...
//			totleBankFee += h.getBankFee();
	...
	public CurrentPage<OrderInfo> queryMerToday(Integer pageNo, String mid, Integer gate, Integer tstat, Integer type,
		...
		return transactionDao.queryMerToday(pageNo,AppParam.getPageSize(),mid,gate,tstat,type, tseq,oid,gid,bkseq,mstate);
	}
	...
	public CurrentPage<OrderInfo> queryHlogDetail(Integer pageNo, String mid, Integer gate, Integer tstat, Integer type,
		...
		return transactionDao.queryHlogDetail(pageNo,AppParam.getPageSize(), 
	...
	public CurrentPage<OrderInfo> querypaymentHlogDetail(Integer pageNo, String mid, Integer gate, Integer tstat,
			String oid, Integer gid, String date, Integer bdate, Integer edate,String tseq,Integer bkCheck,String bkseq,Integer mstate,int type,String batchNo){
				return transactionDao.querypaymentHlogDetail(pageNo, AppParam.getPageSize(), 
	...
	public OrderInfo queryHlogByMer(String mid, Integer mdate, String oid) {
		...
		try {
			...
			LogUtil.printErrorLog("", params, e);
		}
		return hlog;
	}
	/**
	...
	*/
	...
	public CurrentPage<OrderInfo> queryPhonePay(Integer pageNo, String table, String mid, Integer tstat, Integer bdate,
		...
		return transactionDao.queryPhonePay(pageNo, AppParam.getPageSize(), table, mid, tstat, bdate, edate, operid);
	}
	...
	public CurrentPage<OrderInfo> queryCreditCardResult(int pageNo, String mid, int bdate, int edate, Integer tstat, String tseq,
		...
		return transactionDao.queryMlogList(pageNo, AppParam.getPageSize(), mid, bdate, edate, tstat, tseq, cardType, cardVal, payAmount,mstate);
	}
	...
	public CurrentPage<OrderInfo> querypaymentHlogDetail_cz(Integer pageNo, String mid,Integer tstat,
			String date, Integer bdate, Integer edate,String tseq,String oid,Integer mstate,int pstate){
				return transactionDao.querypaymentHlogDetail_cz(pageNo, AppParam.getPageSize(), 
...
//			totleBankFee += h.getBankFee();
	...
	public FileTransfer downloadpaymentDetail_mer(Integer gate, Integer tstat,
	   ...
	   CurrentPage<OrderInfo> hlogListPage=transactionDao.querypaymentHlogDetail(1,-1, mid, gate, tstat, oid, gateRouteId, date, bdate, edate,tseq,bkCheck,bkseq,mstate,type,batchNo);
		...
		String totleAmount=Ryt.div100(hlogListPage.getSumResult().get(AppParam.AMT_SUM).toString());
		String totleSysAmtFee=Ryt.div100(hlogListPage.getSumResult().get(AppParam.SYS_AMT_FEE_SUM).toString());
		list.add("序号,电银流水号,商户号,商户简称,账户号,商户订单号,批次号,交易金额(元),系统手续费(元),交易状态,交易类型,交易银行,订单时间,失败原因"
			.split(","));
		int i = 0;
		for (OrderInfo h : hlogList) {
			String[] str = {
...
//			totleBankFee += h.getBankFee();
		...
		try {
			...
			LogUtil.printErrorLog("downloadpaymentDetail_mer", e);
		}
		return null;
	}
	...
	public CurrentPage<OrderInfo> querypaymentHlogDetail_cz_mer(Integer pageNo,
		...
		return transactionDao.querypaymentHlogDetail_cz(pageNo,
				AppParam.getPageSize(), mid, tstat, date, bdate, edate,
...
//			totleBankFee += h.getBankFee();
	 ...
	 * @return
	 */
	public CurrentPage<AccTradeInfo> queryAcoountCzjlDetail(Integer pageNo, Integer bdate, Integer edate, String oid, Integer state) {
		LogUtil.printInfoLog("充值记录查询--调用账户系统接口");
		String url = null;
		Map<String, Object> maps = new HashMap<String, Object>();
	    //消息版本号
		maps.put("version", 10);
		//交易码
		maps.put("tranCode", "ZH0065");
		//系统标识
		maps.put("sys", "RYF");
		//用户账户号
		String accId = "";
		...
		//订单号
		maps.put("orderId", oid);
		//充值日期
		maps.put("orderDate", bdate);
		//状态
		maps.put("status", state);
		//页码
		maps.put("page", pageNo);
		//页数
		maps.put("pageSize", 15);
		//数字签名 参与数字签名的原字符串： version+tranCode+sys+null+inAccId+transId+bdate+edate+status
	    maps.put("chkValue", MD5.getMD5(("10"+"ZH0065"+"RYF"+ "" + accId + oid + bdate + state).getBytes()));
		LogUtil.printInfoLog("充值记录查询--调用账户系统接口-参数",maps);
		...
		if(str != null && str != ""){
			...
			String pageTotle = object.getString("total");
			if(resCode.equals("000")){
				CurrentPage<AccTradeInfo> page = new CurrentPage<AccTradeInfo>();
		    	...
		    	if(!items.isEmpty()){
		    		...
		    		List<AccTradeInfo> list = new ArrayList<AccTradeInfo>();
		    		for(int i=0;i<item.length;i++){
		    			AccTradeInfo acc = new AccTradeInfo();
		    			//订单编号
		    			acc.setOid(item[0]);
		    			//订单时间
		    			acc.setDate(item[1]);
		    			//交易金额
		    			acc.setAmount(item[2]);
		    			//手续费
		    			acc.setFee(item[3]);
		    			//交易状态
		    			acc.setState(Integer.parseInt(item[5]));
		    			//失败原因
		    			acc.setFailReason(item[6]);
		    			//备注
		    			acc.setRemark(item[7]);
		    			list.add(acc);
		    		}
	 ...
	 * @throws Exception
	 */
	public FileTransfer downloadDetail_Account(int operId, Integer bdate, Integer edate,String oid,Integer state) throws Exception {
		LogUtil.printInfoLog("充值记录下载--调用账户系统接口");
		String url = null;
		Map<String, Object> maps = new HashMap<String, Object>();
	    //消息版本号
		maps.put("version", 10);
		...
		//数字签名 参与数字签名的原字符串： version+tranCode+sys+null+inAccId+transId+bdate+edate+status
	    maps.put("chkValue", MD5.getMD5(("10"+"ZH0065"+"RYF"+ "" + accId + oid + bdate + state).getBytes()));
		LogUtil.printInfoLog("充值记录下载--调用账户系统接口-参数",maps);
		
		ArrayList<String[]> list = new ArrayList<String[]>();
		list.add("序号,商户订单号,充值日期,充值金额(元),系统手续费(元),交易状态,失败原因,备注".split(","));
		...
		if(str != null && str != ""){
			...
			if(resCode.equals("000")){
		    	...
		    	if(!items.isEmpty()){
		    		String[] item = items.split(",");
		    		List<AccTradeInfo> accList = new ArrayList<AccTradeInfo>();
		    		for(int i=0;i<item.length;i++){
		    			AccTradeInfo acc = new AccTradeInfo();
		    			//订单编号
		    			acc.setOid(item[0]);
		    		...
		    		int i = 0;
		    		for (AccTradeInfo a : accList) {
		    			String[] str1 = { (i + 1) + "", a.getOid() + "", a.getDate() + "",
		    					Ryt.div100(a.getAmount()),Ryt.div100(a.getFee()), AppParam.tlog_tstat.get(Integer.parseInt(a.getState() + "")),
		    					a.getFailReason(), a.getRemark() + "" };
		    			i += 1;
		    			list.add(str1);
		    		}
		    		String filename = "ACCOUNTLIST_" + DateUtil.today() + ".xlsx";
		    		String name = "充值记录表";
		    		return new DownloadFile().downloadXLSXFileBase(list, filename, name);
		    	}
			}else{
	 ...
	 * @return
	 */
	public CurrentPage<AccTradeInfo> queryAccountTxjlDetail(Integer pageNo, Integer bdate, Integer edate, String oid, Integer state) {
		LogUtil.printInfoLog("提现记录查询--调用账户系统接口");
		String url = null;
		Map<String, Object> maps = new HashMap<String, Object>();
	    //消息版本号
		maps.put("version", 10);
		//交易码
		maps.put("tranCode", "ZH0066");
		//系统标识
		maps.put("sys", "RYF");
		//用户账户号
		String accId = "";
		...
		maps.put("pageSize", 15);
		//数字签名 参与数字签名的原字符串： version+tranCode+sys+null+inAccId+transId+bdate+edate+status
	    maps.put("chkValue", MD5.getMD5(("10"+"ZH0066"+"RYF"+ "" + accId + oid + bdate + state).getBytes()));
		LogUtil.printInfoLog("提现记录查询--调用账户系统接口-参数",maps);
	 ...
	 * @throws Exception
	 */
	public FileTransfer downloadDetail_TXJL(int operId, Integer bdate, Integer edate,String oid,Integer state) throws Exception {
		LogUtil.printInfoLog("提现记录下载--调用账户系统接口");
		String url = null;
		Map<String, Object> maps = new HashMap<String, Object>();
	    //消息版本号
		maps.put("version", 10);
		...
		//数字签名 参与数字签名的原字符串： version+tranCode+sys+null+inAccId+transId+bdate+edate+status
	    maps.put("chkValue", MD5.getMD5(("10"+"ZH0066"+"RYF"+ "" + accId + oid + bdate + state).getBytes()));
		LogUtil.printInfoLog("提现记录下载--调用账户系统接口-参数",maps);
		
		ArrayList<String[]> list = new ArrayList<String[]>();
		list.add("序号,电银流水号,商户订单号,提现金额(元),手续费(元),实际提现金额,交易状态,订单时间,失败原因,备注".split(","));
		...
		if(str != null && str != ""){
			...
			if(resCode.equals("000")){
		    	...
		    	if(!items.isEmpty()){
		    		...
		    		String filename = "TXJLLIST_" + DateUtil.today() + ".xlsx";
		    		String name = "提现记录表";
		    		return new DownloadFile().downloadXLSXFileBase(list, filename, name);
		    	}
			}else{
 ...
 * @return
 */
public CurrentPage<AccTransferInfo> queryAccTransferInfo(Integer pageNo, String transId,String inAccId, Integer bdate, Integer edate, Integer state) {
	LogUtil.printInfoLog("转账信息查询--调用账户系统接口");
	String url = null;
	Map<String, Object> maps = new HashMap<String, Object>();
    //消息版本号
	maps.put("version", 10);
	//交易码
	maps.put("tranCode", "ZH0067");
	//系统标识
	maps.put("sys", "RYF");
	//交易流水号
	maps.put("transId", transId);
	//转入账号
	maps.put("inAccId", inAccId);
	//bdate 起始日期
	maps.put("beginDate", bdate);
	//edate 结束日期
	maps.put("endDate", edate);
	//状态
	...
	maps.put("pageSize", 15);
	//数字签名 参与数字签名的原字符串： version+tranCode+sys+null+inAccId+transId+bdate+edate+status
    maps.put("chkValue", "10"+"ZH0067"+"RYF"+ "" + inAccId + transId +bdate + edate + state);
	LogUtil.printInfoLog("转账记录查询--调用账户系统接口-参数",maps);
		...
		if(str != null && str != ""){
			...
			String resCode = object.getString("resCode");
			String pageTotle = object.getString("total");
			String transAmt = object.getString("transAmt");
			String feeSum = object.getString("transAmt");
			
			if(resCode.equals("000")){
				CurrentPage<AccTransferInfo> page = new CurrentPage<AccTransferInfo>();
			  	page.setPageTotle(Integer.parseInt(pageTotle));
		    	page.setPageNumber(pageNo);
		    	page.setPageSize(AppParam.getPageSize());
		    	Map<String,Object> map = new HashMap<String,Object>();
		    	map.put(AppParam.AMT_SUM, Ryt.mul100(transAmt));
		    	...
		    	if(!items.isEmpty()){
		    		String[] item = items.split(",");
		    		List<AccTransferInfo> list = new ArrayList<AccTransferInfo>();
		    		
		    		for(int i=0;i<item.length;i++){
		    			AccTransferInfo acc = new AccTransferInfo();
		    			//交易流水号
		    			acc.setTransId(item[0]);
		    			//转账金额(元)
		    			acc.setTransAmt(item[2]);
		    			//手续费(元)
		    			acc.setFee(item[3]);
		    			//交易对方账号
		    			acc.setTrader(item[4]);
		    			//转账时间
		    			acc.setTransTime(item[1]);
		    			//转账状态
		    			acc.setState(Integer.parseInt(item[5]));
		    			//备注
		    			acc.setRemark(item[6]);
		    			list.add(acc);
		    		}
		    		page.setPageItems(list);
		    		return page;
 ...
 * @return
 */
public FileTransfer downloadDetail_ZZJL(int operId, String transId,String inAccId, Integer bdate, Integer edate, Integer state) throws Exception {
	LogUtil.printInfoLog("转账记录下载--调用账户系统接口");
	String url = null;
	Map<String, Object> maps = new HashMap<String, Object>();
    //消息版本号
	maps.put("version", 10);
	...
	//数字签名 参与数字签名的原字符串： version+tranCode+sys+null+inAccId+transId+bdate+edate+status
    maps.put("chkValue", "10"+"ZH0067"+"RYF"+ "" + inAccId + transId +bdate + edate + state);
	LogUtil.printInfoLog("转账记录下载--调用账户系统接口-参数",maps);
	
	ArrayList<String[]> list = new ArrayList<String[]>();
	list.add("序号,商户订单号,转账金额(元),手续费(元),交易对方账户,转账时间,转账状态,备注".split(","));
	...
	if(str != null && str != ""){
		...
		if(resCode.equals("000")){
	    	...
	    	if(!items.isEmpty()){
	    		String[] item = items.split(",");
	    		List<AccTransferInfo> accList = new ArrayList<AccTransferInfo>();
	    		for(int i=0;i<item.length;i++){
	    			AccTransferInfo acc = new AccTransferInfo();
	    			//交易流水号
	    			acc.setTransId(item[0]);
	    		...
	    		int i = 0;
	    		for (AccTransferInfo a : accList) {
	    			String[] str1 = { (i + 1) + "", a.getTransId() + "", 
	    					Ryt.div100(a.getTransAmt()),Ryt.div100(a.getFee()),a.getTrader() + "" ,a.getTransTime() + "",
	    					AppParam.tlog_tstat.get(Integer.parseInt(a.getState() + "")),a.getRemark() + "" };
	    			i += 1;
	    			list.add(str1);
	    		}
	    		String filename = "ZZJLLIST_" + DateUtil.today() + ".xlsx";
	    		String name = "转账记录表";
	    		return new DownloadFile().downloadXLSXFileBase(list, filename, name);
	    	}
		}else{
