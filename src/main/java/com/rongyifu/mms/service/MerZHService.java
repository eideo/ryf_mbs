...
import com.rongyifu.mms.bean.AccPayDetail;
...
public class MerZHService {
	...
	public String uploadBatchPic(String uid,String trans_flow,String oid,FileTransfer fileTransfer,String recharge_amt ){
		try {
			...
			LogUtil.printErrorLog("上传充值凭证失败-"+e.getMessage());
			return "上传失败";
		}
...
//				file1.createNewFile();
			...
			LogUtil.printErrorLog("file=" + file + ", fileName=" + fileName, e);
		}
	...
	public FileTransfer downloadMX(Integer ptype,Integer bdate,Integer edate,Integer state,String oid) throws Exception {
		...
		try {
			...
			LogUtil.printErrorLog("ptype=" + ptype + ", bdate=" + bdate + ", edate=" + edate + ", state=" + state + ", oid=" + oid, e);
		}
	 ...
	 * @throws ParseException 
	   */
	public CurrentPage<AccSeqs> queryLS(Integer pageNo,Integer bdate,Integer edate){
		LogUtil.printErrorLog("商户流水查询接口");
		if(null == pageNo){
			return null;
		}
		String url = ParamCache.getStrParamByName("QSXT_URL") + "backstagemamage" + "/reciveMerStanceService?"; 
		Map<String,Object> maps = new HashMap<String,Object>();
		maps.put("version", "10");
		maps.put("tranCode", "ZH0005");
		//商户号
		maps.put("mer_code", session.getAttribute(WebConstants.SESSION_LOGGED_ON_MID));
		//查询起始日期
		maps.put("start_date", bdate);
		//查询结束日期
		maps.put("end_date", edate);
		//页码
		maps.put("pageNo", pageNo);
		//页数
		maps.put("pageNum", AppParam.getPageSize());
		LogUtil.printInfoLog("商户流水查询接口-参数",maps);
		...
		if(str != null && str !=""){
			...
			if(resCode.equals("000")){
				CurrentPage<AccSeqs> page = new CurrentPage<AccSeqs>();
		    	...
		    	//成功情况下，获取清算系统返回的值并进行解析
		    	String items = json.getString("items");
		    	if(!items.isEmpty()){
		    		String[] item = items.split("\\|");
		    		List<AccSeqs> list = new ArrayList<AccSeqs>();
		    		for(int i=0;i<item.length;i++){
		    			String data = item[i].toString();
		    			AccSeqs acc = new AccSeqs();
		    			String[] accSeqs = data.split(",");
		    			//商户号
		    			acc.setUid(accSeqs[0]);
		    			//系统日期
		    			acc.setTrDate(Integer.parseInt(accSeqs[1].substring(0, 8)));
		    			//系统时间
		    			acc.setTrTime(Integer.parseInt(accSeqs[1].substring(8, 14)));
		    			//交易金额
		    			acc.setTrAmt(Long.parseLong(Ryt.mul100(accSeqs[2])));
		    			//交易手续费
		    			acc.setTrFee(Integer.parseInt(Ryt.mul100(accSeqs[7])));
		    			//变动金额
		    			acc.setAmt(Long.parseLong(Ryt.mul100(accSeqs[3])));
		    			//余额
		    			acc.setAllBalance(Long.parseLong(Ryt.mul100(accSeqs[4])));
		    			//电银流水号
		    			acc.setTbId(accSeqs[5]);
		    			//简短说明
		    			acc.setRemark(accSeqs[6]);
		    			list.add(acc);
		    		}
	 ...
	 * @throws Exception
	 */
	public FileTransfer downloadLS(String operId,Integer bdate,Integer edate) throws Exception {
		 LogUtil.printInfoLog("商户流水查询--下载");
		String url = ParamCache.getStrParamByName("QSXT_URL")+ "backstagemamage/" + "merFundStanceDataDownLoadService?";
		Map<String,Object> maps =new  HashMap<String,Object>();
		maps.put("version", "10");
		maps.put("tranCode", "ZH0011");
		//商户号
		maps.put("mer_code", session.getAttribute(WebConstants.SESSION_LOGGED_ON_MID));
		//查询起始日期
		maps.put("start_date", bdate);
	    ...
	    maps.put("merPriv", operId + "," +13 +"," + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()));
		LogUtil.printInfoLog("商户流水查询接口-参数",maps);
		String str = Ryt.requestWithPost(maps, url);
...
//		}
	...
	public String editOneCusContactInfos(Integer id,String contact,String position,String cell){
		if(merZHDao.editOneCusContactInfos(id,contact,position,cell)==1)
			return "修改成功";
		else
			return "修改失败";
	}
	...
	public String addOneCusContactInfos(String iiUid,String iContact,String iPosition,String iCell){
		...
		if(merZHDao.addOneCusContactInfos(iiUid,iContact,iPosition,iCell)==1)
			return "操作成功";
		else
			return "操作失败";
	}
...
//			long balance=accInfos.getBalance();//账户可用余额 格式 (以分为单位)
		...
		public List<AccInfos> queryJSZHYEByMid(String mid){
			LogUtil.printInfoLog("商户余额查询开始");
			String url = ParamCache.getStrParamByName("QSXT_URL")+"backstagemamage/"+"merchantBalanceQueryService";
			Map<String, Object> reqPramas = new HashMap<String, Object>();
			reqPramas.put("tranCode", "ZH0004");
			reqPramas.put("version", "10");
			reqPramas.put("mer_code", mid);
			LogUtil.printInfoLog("商户余额查询参数", reqPramas);
			LogUtil.printInfoLog("请求参数", reqPramas);
			...
			if(resStr !=null && resStr!=""){
			    ...
			    String resCode=json.getString("resCode");
			     ...
			     if(resCode.equals("000")){
					//成功
			    	 String merBalance=json.getString("mer_balance");
			    	 String merState=json.getString("mer_state");
			    	 List<AccInfos> myList =new ArrayList<AccInfos>();
			    	 AccInfos acc=new AccInfos();
			    	 acc.setUid(mid);
			    	 acc.setAid(mid);
			    	 acc.setState(Short.parseShort( merState));
			    	 myList.add(acc);
			    	//System.out.println(Long.parseLong(Ryt.mul100(merBalance)));
			    	 acc.setBalance(Long.parseLong(Ryt.mul100(merBalance)));
			    	 return myList;
		...
		public List<AccInfos> getAccBalance(String accId){
			LogUtil.printInfoLog("账户余额查询开始");
			String url="";
			
			Map<String, Object> reqPramas = new HashMap<String, Object>();
			//协议参数
			reqPramas.put("tranCode", "ZH0062");
			reqPramas.put("version", "10");
			reqPramas.put("sys", "RYF");
			reqPramas.put("accId", accId);
			
			String timestamp= DateUtil.getNowDateTime();
			reqPramas.put("timestamp", timestamp);
			String chkValue ="10"+"ZH0062"+"RYF"+accId+timestamp;
			reqPramas.put("chkValue", chkValue);
			LogUtil.printInfoLog("请求参数", reqPramas);
			try {
				String resStr=Ryt.requestWithPost(reqPramas,url);
				if(resStr !=null && resStr!=""){
				    ...
				    String resCode=json.getString("resCode");
				     if(resCode.equals("000")){
				    	//成功
				    	 List<AccInfos> myList =new ArrayList<AccInfos>();
				    	 AccInfos acc=new AccInfos();
				    	 String accId1=json.getString("accId");
				    	 String balance=json.getString("balance");//总余额
				    	 String avaBalance=json.getString("avaBalance");//可用余额
				    	 String frzBalance=json.getString("frzBalance");
				    	 acc.setAid(accId1);
				    	 acc.setBalance(Long.parseLong(Ryt.mul100(avaBalance)));//可用余额
				    	 acc.setAllBalance(Long.parseLong(Ryt.mul100(balance)));//总余额
				    	 acc.setFreezeAmt(Long.parseLong(Ryt.mul100(frzBalance)));//冻结金额
				    	 myList.add(acc);
				    	 return myList;
					}
				     else{
		 ...
		 * @return
		 */
		public String updatePass(String accId,  String opass,String npass,String rePwd){
			LogUtil.printInfoLog("密码修改开始");
			String url="";
			String pwdType ="1";
			Map<String, Object> reqPramas = new HashMap<String, Object>();
			//协议参数
			reqPramas.put("tranCode", "ZH0063");
			reqPramas.put("version", "10");
			reqPramas.put("sys", "RYF");
			reqPramas.put("accId", accId);
			reqPramas.put("oldPwd", opass);
			reqPramas.put("newPwd", npass);
			reqPramas.put("rePwd", rePwd);
			reqPramas.put("pwdType", pwdType);
			String chkValue ="10"+"ZH0063"+"RYF"+accId+pwdType+opass+npass+rePwd;
			reqPramas.put("chkValue", chkValue);
			LogUtil.printInfoLog("请求参数", reqPramas);
			String resStr=Ryt.requestWithPost(reqPramas,url);
			if(resStr !=null && resStr!=""){
		 ...
		 * @return
		 */
		public List<AccPayDetail> queryAccPayDetail(Integer pageNo,Integer startDate,Integer endDate,Integer flag,String tseq,Integer type,String accId){
			LogUtil.printInfoLog("账户收支明细查询开始");
			String url = "";
			Map<String, Object> reqPramas = new HashMap<String, Object>();
			reqPramas.put("tranCode", "ZH0064");
			reqPramas.put("version", "10");
			reqPramas.put("sys", "RYF");
			reqPramas.put("accId", accId);
			reqPramas.put("startDate", startDate);
			reqPramas.put("endDate", endDate);
			reqPramas.put("flag", flag);
			reqPramas.put("type", type);
			reqPramas.put("page", pageNo);
			String chkValue="10"+"ZH0064"+"RYF"+accId+startDate+endDate+flag+type;
			reqPramas.put("chkValue", chkValue);
			LogUtil.printInfoLog("账户收支明细查询参数", reqPramas);
			try {
				String resStr=Ryt.requestWithPost(reqPramas,url);
				CurrentPage<AccPayDetail> currentPage = new CurrentPage<AccPayDetail>();
				if(resStr !=null && resStr!=""){
					//解析
					JSONObject json=JSONObject.fromObject(resStr);
				    String resCode=json.getString("resCode");
				     ...
				     if(resCode.equals("000")){
						//成功
				    	 int totalSize = json.getInt("totalSize");//总页数
				    	 int total = json.getInt("total");//总条数
				    	 int pageSize = json.getInt("pageSize");//每页条数
				    	 JSONArray jsonArray11 =json.getJSONArray("items");
				    	 List listDetails= (List)JSONArray.toList(jsonArray11, AccPayDetail.class);
				    	 CurrentPage<AccPayDetail> page=new CurrentPage<AccPayDetail>();
					    	page.setPageItems(listDetails);
					    	page.setPageTotle(totalSize);
					    	page.setPageSize(pageSize);
					    	page.setPageNumber(pageNo);
					    	page.setPagesAvailable(pageNo);
					    	//return page;
					}
				     return null;
				}
		...
		// 下载
				public FileTransfer downloadAccPayDetail(Integer startDate,Integer endDate,Integer flag,String tseq,Integer type,String accId)throws Exception {
					
					ArrayList<String[]> list = new ArrayList<String[]>();
					list.add("交易流水号,交易时间,收支类型,交易金额(元),手续费(元),变动金额(元),可用余额(元),冻结金额(元),总余额(元),交易类型".split(","));
					...
					String chkValue="10"+"ZH0064"+"RYF"+accId+startDate+endDate+flag+type;
					reqPramas.put("chkValue", chkValue);
					LogUtil.printInfoLog("账户收支明细下载参数", reqPramas);
					String filename = "ACCPAYDEL_" + DateUtil.today() + ".xlsx";
					String name = "当天交易表";
					
					try {
						String resStr=Ryt.requestWithPost(reqPramas,url);
						List<AccPayDetail> downList = new ArrayList<AccPayDetail>();
						if(resStr !=null && resStr!=""){
							//解析
							JSONObject json=JSONObject.fromObject(resStr);
						    String resCode=json.getString("resCode");
						     ...
						     if(resCode.equals("000")){
								//成功
						    	 JSONArray jsonArray11 =json.getJSONArray("items");
						    	 List listDetails= (List)JSONArray.toList(jsonArray11, AccPayDetail.class);
						    	 return new DownloadFile().downloadXLSXFileBase(listDetails, filename, name);
							}
