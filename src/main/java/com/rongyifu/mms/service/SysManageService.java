...
public class SysManageService {
	...
	*/ 
	...
	public String checkBatchChangeRouteByGate(List<Integer> ids,Integer status){
		String msg = "FAIL";
		try {
			dao.checkBatchChangeRouteByGate(ids,status);
			if(1 == status){//审核成功
				//审核成功后刷新缓存
	...
	public CurrentPage<Gate> showBatchRouteApply(Integer transMode,Integer rytGate, Integer pageNo ){
		return dao.queryApplyPage(pageNo, AppParam.getPageSize(),transMode, rytGate);
	}
...
//			return dao.getMessageById(id);
			...
			requestParaMap.put("serviceName", "RMI_MMSNOTICE");
			...
			MMSNotice mmsNotice = (MMSNotice) JSONObject.toBean(jsonObj,MMSNotice.class);
			return mmsNotice;
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
		}
