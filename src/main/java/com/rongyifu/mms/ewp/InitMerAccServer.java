...
public class InitMerAccServer{
	...
	public  void initMerAcc(){
		...
		LogUtil.printInfoLog("init success");
	...
	protected String saveMerAccounts(int previousEDate, int previousBDate) {
		...
		if(delRes>0){
			LogUtil.printInfoLog("delete data  success");
		}else{
			LogUtil.printInfoLog("delete data  fail ,error msg:"+delMsg);
		...
		if(result>0){
			LogUtil.printInfoLog("save success");
			return "success";
		}else{
			LogUtil.printErrorLog("save fail :"+msg);
