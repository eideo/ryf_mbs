...
public class CallRyfPayProcessor {
	...
	public static boolean RecvSyncResult(String respXml){
		try {
			...
			LogUtil.printErrorLog("解析同步返回报文失败,RespXml:"+respXml, e);
			return false;
		}
	...
	public static String RecvRyfPayResult(Map<String, String> p){
		...
		if(!chkValue.equals(sign)){ //验证签名
			LogUtil.printInfoLog("验证签名失败，sign:"+sign+"|chkValue:"+chkValue);
			return "verify_fail";
		}
	...
	private static String requestRyfDf(String reqFun,Map<String, Object> params){
		...
		if(Ryt.empty(url)){
			LogUtil.printErrorLog("tseq="+orderId+"  url is null,",logMap);
			return null;
		}
		...
		logMap.put("url", url);
		LogUtil.printInfoLog("params:", logMap);
		return result;
	}
     ...
     */
     public static String CallRyfPay(TlogBean tlog){
        ...
        try {
				...
				LogUtil.printErrorLog("purpose base64.decode is error,tseq:"+tlog.getTseq(), e);
				purpose="";
			}
...
//            String msg=status.elementText("msg");
     ...
     public static String[] getRespMsg(String respXml){
		...
		try {
			...
			LogUtil.printErrorLog("xml:" + respXml, e);
			resp[0] = "fail";
			resp[1] = "解析返回报文错误";
			return resp;
		}
