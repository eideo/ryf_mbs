...
public class RefundSyncJob {
	...
	public void execute() {
		...
		if(refundInfoList.size() > 0){
			...
			LogUtil.printInfoLog("[start_" + rand + "] list size:"+refundInfoList.size());
			// 清算系统退款信息同步地址
			...
			for(String item : refundInfoList){
				try {
					...
					if("success".equalsIgnoreCase(ifaceResult))
						...
						LogUtil.printInfoLog("sync fail: refundInfo="+item+" resInfo：\n"+resInfo);
						handleSyncFail(item, list);
					}
				} catch (DocumentException e) {
					LogUtil.printErrorLog("RefundInfo="+item, e);
				}
			}
			
			LogUtil.printInfoLog("[end_" + rand + "] sync result: "+ list.size() + "/" + refundInfoList.size() + " syncFailMap："+syncFailMap.size());
	...
	private boolean sendWarnMail(String content){
    	...
    	try {
    		...
    		LogUtil.printErrorLog(content, e);
    	}
