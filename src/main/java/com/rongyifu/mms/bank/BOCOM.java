...
//		return "";
	...
	public static String verifyB2C(Map<String, String> m) throws Exception {
		...
		if (veriyCode < 0) {
			LogUtil.printErrorLog("verifyB2C: notifyMsg=" + notifyMsg);
...
//		String corporNo=B2BClient.getElement("CorporNo");//企业客户号
		...
		LogUtil.printInfoLog("genB2BWebSignString req:"+reqData);
		if (!ret) {
			LogUtil.printErrorLog("genB2BWebSignString jt b2b err:" + B2BClient.getLastErr());
		...
		try {
			...
			LogUtil.printErrorLog("genB2BWebSignString jt b2b occur exception", e);
	...
	public static String genB2BVerifyString(Map<String, String> m) throws Exception {
		...
		if (!ret) {
			...
			LogUtil.printErrorLog("genB2BVerifyString bocom b2b err:" + errmsg);
		...
		try{
			...
			LogUtil.printInfoLog("genB2BVerifyString jt notifyMsg:" + notifyMsg);
...
//		Ryt.print("---jt sign-ok---");
	...
	public static String createBCMPreOrder(Map<String, String> m) throws Exception {
		...
		if (ret != 0) { // 初始化失败
			LogUtil.printErrorLog("createBCMPreOrder ret=" + ret + " lastErr=" + client.getLastErr());
			throw new Exception(client.getLastErr());
		}
		...
		if (rep == null) {
			LogUtil.printErrorLog("createBCMPreOrder error msg:"+client.getLastErr());
			throw new Exception(client.getLastErr());
		} else {
