...
public class DaifuProcessor implements ITaskProcessor{
	...
	@Override
	public void process() {
		LogUtil.printInfoLog("订单[" + o.getOid() + "]付款开始");
		...
		try {
			...
			if (ret.isSucc()) {
				...
				if (ret.getTransStatus() == 2) { 
					...
					LogUtil.printInfoLog("订单[" + o.getOid() + "]付款处理中，等待查询接口获取付款结果！");
				} else {
...
//				dao.batchSqlTransaction(slqs,"type");
				...
				LogUtil.printInfoLog("代付成功", logMap);
			} else {
...
//				dao.batchSqlTransaction(sqls);
				...
				LogUtil.printInfoLog("代付失败", logMap);
			}
		} catch (Exception e) {
			LogUtil.printErrorLog("", e);
		}
		LogUtil.printInfoLog("订单[" + o.getOid() + "]付款完成");
	}
	...
	public synchronized static void execBatchTrans(String[] sqls,String type){
		try {
			...
			LogUtil.printErrorLog("代付同步事务异常", logMap, e);
		}
	}
