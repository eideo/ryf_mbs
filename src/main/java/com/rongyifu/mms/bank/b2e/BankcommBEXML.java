	...
	public void parseXML(B2ERet result,String xmlString) throws B2EException {
		...
		try {
			...
			if (body == null && !tr_code.equals("210201")){
				LogUtil.printErrorLog("parseXML 返回报文没有Body");
			...
			if(tr_code.equals("210201")){
				if(result.isSucc()){
					LogUtil.printInfoLog("parseXML 交易码210201 交易成功");
					result.setTransStatus(2);//设置成处理中 
					return;
				}else{
					LogUtil.printInfoLog("parseXML 交易码210201 交易失败");
					return;
				}
			}
			// 查询余额
...
//					result.setErr("状态未知");
			...
			if (tr_code.equals("330002")) {
				...
				LogUtil.printInfoLog("parseXML sucFlag:"+sucFlg+"<<<<");
...
//					result.setStatusInfo("交易失败");
					...
					LogUtil.printInfoLog("parseXML 订单["+reqNo+"]付款结果：交易可疑");
					...
					LogUtil.printInfoLog("parseXML 订单["+reqNo+"]付款结果：交易成功");
					result.setSucc(true);
					Element serialNo=head.element("serial_no");//银行流水
					result.setResult(serialNo.getText());
					...
					LogUtil.printInfoLog("parseXML 订单["+reqNo+"]付款结果：交易失败");
					result.setSucc(false);
					result.setTransStatus(1);
					result.setStatusInfo("交易失败");
				} else if("34".equals(tranStatus)){
					LogUtil.printInfoLog("parseXML 订单["+reqNo+"]付款结果：转账申请已受理，正在处理中");
					Element serialNo=head.element("serial_no");//银行流水
					result.setResult(serialNo.getText());
					result.setSucc(true);
					...
					LogUtil.printInfoLog("parseXML 订单["+reqNo+"]付款结果：未知状态");
					Element serialNo=head.element("serial_no");//银行流水
					result.setResult(serialNo.getText());
					result.setSucc(true);
					result.setTransStatus(2);
