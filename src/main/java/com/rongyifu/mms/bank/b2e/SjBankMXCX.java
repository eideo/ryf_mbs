...
public class SjBankMXCX {
	...
	public List<String>  querySjBankDetail(){
		...
		try {
			...
			LogUtil.printInfoLog("reqData:"+reqData);
			String respData=sjBankSocket(reqData);
			LogUtil.printInfoLog("respData:"+respData);
			parseXml(respData);
			...
			if(record_num!=null && record_num > 0){
				if("1".equals(fileFlag)){ //文件返回方式
					...
					if(!downFileFromHost()){
						LogUtil.printErrorLog("FTP下载文件失败！从前置拉取文件失败");
			...
			LogUtil.printInfoLog("params", map);
		} catch (Exception e) {
			LogUtil.printErrorLog(e.getMessage(), map, e);
		}
	...
	private void parseXml(String xml){
		...
		try {
			...
			if(head==null || body==null){
				LogUtil.printErrorLog("返回报文错误："+xml);
				return;
			}
	...
	private List<String> readFile(String fileName) throws Exception{
		...
		reader.close();
		if(file.delete()){
			LogUtil.printInfoLog("成功删除文件！"+fileName);
		}
		return ls;
	}
