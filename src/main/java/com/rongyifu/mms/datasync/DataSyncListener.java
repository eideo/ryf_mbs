						...
						for (File item : files) { // 遍历指定目录
							...
							if (item.isFile()) { // 判断是否为文件
								...
								if (!Ryt.empty(fileName) && fileName.endsWith(syncFile.getFileSuffix()) && len == syncFile.getFileNameLen()) {
									LogUtil.printInfoLog("开始解析文件：" + syncFile.getFilePath() + item.getName());
									
									syncFile.setFileName(fileName);
									ParseSyncData psd = new ParseSyncData();
									psd.parseData(syncFile);
									
									LogUtil.printInfoLog("文件解析完成：" + syncFile.getFilePath() + item.getName());
									
									// 删除文件
									if(item.delete())
										LogUtil.printInfoLog("成功删除文件：" + syncFile.getFilePath() + item.getName());
									else
										LogUtil.printInfoLog("文件删除失败：" + syncFile.getFilePath() + item.getName());
								}
							}
						}
					}
