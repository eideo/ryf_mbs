...
public class PosDzSyncListener implements Runnable  {
	...
	public void process(){
		try{
			...
			LogUtil.printErrorLog("exception:"+e.getMessage(), e);
		}
	}
		...
		try{
			...
			for (SyncFileInfo syncFile : getSyncFile()) {
				if(fileSuffix.equals(syncFile.getFileSuffix())){
					LogUtil.printInfoLog("fileSuffix:"+fileSuffix);
	...
	private void loopReadFile(List<SyncFileInfo> sfL){
		...
		try {
			...
			//文件不存在 报警
			if(fl.size()==0){
				LogUtil.printInfoLog("三次遍历目录，文件仍然不存在，请手工上传");
	...
	private void keyCode(SyncFileInfo sync){
		...
		LogUtil.printInfoLog("开始解析文件：" + DataSyncUtil.PosDz_FILE_PATH + fileName);
		
		ParseSyncDataForPosDz psd = new ParseSyncDataForPosDz();
		psd.parseData(sync);
		
		LogUtil.printInfoLog("文件解析完成：" + DataSyncUtil.PosDz_FILE_PATH + fileName);
		
		// 删除文件
		if(file.delete())
			LogUtil.printInfoLog("成功删除文件：" + DataSyncUtil.PosDz_FILE_PATH + fileName);
		else
			LogUtil.printInfoLog("文件删除失败：" + DataSyncUtil.PosDz_FILE_PATH + fileName);
	}
	...
	protected  boolean isExec(){
		...
		LogUtil.printInfoLog("预设执行时间："+preTime+"	预设执行状态："+isOpen);
		//开启状态为1 不执行
		if("1".equals(isOpen)){
			LogUtil.printInfoLog("执行状态未开启");
			return true;
		}
		
		//预设执行时间不符 不执行
		if(!nowTime.equals(preTime)){
			LogUtil.printInfoLog("未到预设的执行时间");
			return true;
		}
		return false;
	...
	private  void isExistsFile(int i,List<String> fl,String path) throws InterruptedException{
		if(i>3)return;
		LogUtil.printInfoLog("开始执行清算对账数据同步_"+i);
