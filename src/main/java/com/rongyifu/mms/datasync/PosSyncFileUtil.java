...
//				 content.append(boid).append(",");//原支付流水（20140224版本更新去掉）
	...
	public List<OrderInfo> getDateFromHlog(String dates,String where){
		...
		if(!Ryt.empty(where))
			...
			LogUtil.printInfoLog("Sql条件语句为空!");
			return new ArrayList<OrderInfo>();
		}
		LogUtil.printInfoLog("执行Sql:"+sql.toString());
	...
	public static List<Object> getData(String methodName,String where,String date){
		...
		try {
			...
			records=(List<Object>)method.invoke(PosSyncFileUtil.class.newInstance(), date, where);
		} catch (Exception e) {
			LogUtil.printErrorLog("获取数据库记录出错:"+methodName);
		}
		return records;
	}
...
//		arg[0]=records;
		try {
			Method method= PosSyncFileUtil.class.getMethod(methodName, paramType);
			datas=(List<String>)method.invoke(PosSyncFileUtil.class.newInstance(),records);
		} catch (Exception e) {
			e.printStackTrace();
			LogUtil.printErrorLog("处理数据异常:"+methodName);
		}
		return datas;
	}
	...
	public static String getDays(String bdate,String edate){
		...
		try {
			...
			LogUtil.printInfoLog("时间处理异常,错误原因:"+e.getMessage());
		}
		...
		try{
			if(datas.size()>0){
				if (datas.get(0).equals("none") || "".equals(datas.get(0).trim())){
					LogUtil.printInfoLog("代理系统数据采集---该文件当天已生成");
					return true;
				}
				...
				osw.flush();
				LogUtil.printInfoLog("代理系统数据采集---文件生成成功，共 "+count+" 条数据，文件："+fileName);
				...
				if(dayCount !=0 && dayCount !=1)
					...
					LogUtil.printInfoLog("代理系统数据采集---数据为空,当前为周日或者周一");
			}
			return true;
		}catch (Exception e) {
			DataSyncDb.recordLog(dao, PRO_TYPE, fileName, false,logDate);
			DataSyncUtil.sendMail(e.getMessage(), PRO_TYPE, fileName);
			LogUtil.printErrorLog("代理系统数据采集---生成文件异常", e);
			e.printStackTrace();
			return false;
		}finally{
			try{
