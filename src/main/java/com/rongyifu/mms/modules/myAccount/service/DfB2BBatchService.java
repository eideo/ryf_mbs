			...
			if (accInfos.getState() != 1) {
  			...
  			for (int i = 1; i < rows; i++) {
  				...
  				try{
  					...
  					LogUtil.printErrorLog("mid=" + uid + " gateId=" + gateId + " errorMsg=" + e.getMessage(), e);
  			...
  			LogUtil.printErrorLog("批量处理异常："+e.getMessage(), e);
...
//			String toBkName=bkInfo[1];
		...
		synchronized (lock) {
			...
			try{
				...
				LogUtil.printErrorLog("批次号 " + batchNo + " 支付失败", e);
