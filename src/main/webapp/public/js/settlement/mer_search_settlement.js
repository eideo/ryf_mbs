    ...
    //提取用户列表的回设函数：callSettlementList中放的是FeeLiqBath对象
	     function callSettlementList(SettlementList) {
			    ...
			    var cellFuncs = [
			         ...
			         var str = "<input type='button' class='button' value='查看明细' onclick=\"queryDetail()\">&nbsp;&nbsp;&nbsp;"
           			+ "<input type='button' class='button' value='下载Excel明细' onclick=\"downLoadXLS('FormOne','"+mid+"','"+operId+"','"+date_b+"','"+date_e+"','qhlog')\">";
...
for (var i = 0; i < SettlementTwoList.length; i++) {
  ...
  /* var batchTd = elTr.insertCell(-1);
   ...
   var orgOid = elTr.insertCell(-1);
   orgOid.innerHTML = theHlog.orgOid;
}
...
function queryDetail(){
	...
	if(batch==undefined || batch=='')
		...
		MerSettlementService.queryHlogs(batch,callSettlementTwoList);
	}
}
//下载
function downLoadXLS(formName,mid,operId,bdate,edate,action){ 
	var batch='';
	batchs=document.getElementsByName("chkedrd");
	for(var i=0;i<batchs.length;i++){
		if(batchs[i].checked==true){
			batch=batchs[i].value;
		}
  ...
  if(batch==undefined || batch==''){
      ...
      return false;
		...
		MerSettlementService.downloadSettleDetail(p,function(data){
