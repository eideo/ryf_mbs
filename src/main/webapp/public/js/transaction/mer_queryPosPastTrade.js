...
function onlyNu(o){
 ...
 function queryMerToday(pageNo){
	 ...
	 QueryPosMerTodayService.queryMerHlogs(pageNo,15,mid,deduct_sys_stance,innerTermId,type,tstat,beginDate,endDate,xpe_deduct_refer,callBack2);	
 ...
 var callBack2 = function(pageObj){
	  $("merTodayTable").style.display="";
	   ...
	   var cellFuncs = [
	                    ...
	                    function(obj) { 
	                    return "<input type=\"button\" value=\" 详情 \" onclick=\"queryHlogDetails('" + obj.tseq+ "')\" >";
	                    }
	                  ];
 ...
 //点击详情，查看当前商户的详细信息
   function queryHlogDetails(tseq){
	   //商户号 
       ...
       jQuery("#detailHlogInfo").wBox({title:"&nbsp;&nbsp;pos历史交易查询信息详情",show:true});//显示box
   }
...
function downloadDetail(operId){
	 ...
	 QueryPosMerTodayService.downloadHlog(operId, mid,deduct_sys_stance,innerTermId,type,tstat,beginDate,endDate,xpe_deduct_refer,function(data){
	      	if (data == null) {
			alert("稍后到下载页面下载数据！");
		}
});
